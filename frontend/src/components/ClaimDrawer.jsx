...
function DecisionSupport({claimId,brief,briefLoading,documents,claim,notify}){
  ...
  return(
    <>
      ...
      {brief&&<>
        ...
        {(brief.actions||[]).map((a,i)=>(
          <div key={a.diary_id||i} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 16px",marginBottom:12}}>
            ...
            {a.diary_id&&(confirming?.diaryId!==a.diary_id
      ...
      {claim&&claim.status==='settlement_discussions'&&(
        <div style={{marginTop:22,background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"13px 16px"}}>
          <div style={{fontSize:10,fontFamily:C.mono,fontWeight:700,color:C.muted,letterSpacing:".06em",marginBottom:8}}>SETTLEMENT PACKAGES (DWC 10214 — DRAFT RENDITIONS)</div>
          <div style={{display:"flex",gap:8}}>
            <Btn small onClick={()=>genPackage('cnr')}>Generate C&R package</Btn>
            <Btn small variant="ghost" onClick={()=>genPackage('stip')}>Generate Stip package</Btn>
          </div>
        </div>
      )}
      ...
      <div style={{marginTop:26}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <Btn small variant="ghost" onClick={()=>setIngesting(v=>!v)}>+ Ingest document</Btn>
        </div>
        {ingesting&&(
