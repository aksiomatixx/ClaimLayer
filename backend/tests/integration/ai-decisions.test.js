...
async function seedDecisions() {
  for (const r of [
    { id: '11111111-1111-4111-8111-111111111111', claim_id: 'A', decision_type: 'compensability', prompt_name: 'compensability_analysis',  model: 'claude-sonnet-4-6', input_snapshot: { x: 1 }, output_parsed: { compensability: 'Likely Compensable' }, input_tokens: 800, output_tokens: 600, latency_ms: 3500, confidence: 88, guardrail_actions: [],                                          created_at: new Date().toISOString() },
    { id: '22222222-2222-4222-8222-222222222222', claim_id: 'A', decision_type: 'rfa_mtus',       prompt_name: 'rfa_mtus_evaluation',     model: 'claude-sonnet-4-6', input_snapshot: { y: 2 }, output_parsed: { recommendedAction: 'auto_approve' }, input_tokens: 600, output_tokens: 400, latency_ms: 2200, confidence: 90, guardrail_actions: [{ rule: 'no_auto_deny', triggered: true }], human_decision: 'adjuster_approved', human_decision_at: new Date().toISOString(), created_at: new Date().toISOString() },
    { id: '33333333-3333-4333-8333-333333333333', claim_id: 'B', decision_type: 'cnr_pricing',    prompt_name: 'cnr_pricing',             model: 'claude-sonnet-4-6', input_snapshot: { z: 3 }, output_parsed: { cnrValueMid: 27500 }, input_tokens: 1200, output_tokens: 900, latency_ms: 5800, confidence: null, guardrail_actions: [{ rule: 'cnr_premium_cap_1.15x', triggered: true, action: 'flagged_above_premium_threshold' }], created_at: new Date().toISOString() },
  ]) {
