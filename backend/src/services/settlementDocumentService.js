...
async function _renderPackage(formName, claim, sections) {
  ...
  let y = 744;
  // Page-overflow handling: long dispute lists / breakdowns continue on
  // a fresh page (with the draft watermark repeated) instead of drawing
  // off the bottom edge.
  const newPage = () => {
    page = pdf.addPage([612, 792]);
    y = 744;
    page.drawText(DRAFT_WATERMARK, { x: 50, y, size: 8.5, font: bold, color: red });
    y -= 22;
  };
  const line = (text, f, size, color) => {
    for (const para of String(text).split('\n')) {
      ...
      const flush = () => {
        if (y < BOTTOM) newPage();
        page.drawText(cur, { x: 50, y, size, font: f, color });
        y -= size + 5;
      };
  ...
  for (const [heading, rows] of sections) {
    if (y < BOTTOM + 40) newPage(); // keep a heading with its first rows
  ...
  return { buffer: Buffer.from(await pdf.save()), pages: pdf.getPageCount() };
}

async function _fileVersioned(claimId, title, rendered, kind) {
  const now = new Date().toISOString();
  const { data: priors, error: pErr } = await supabase
    .from('claim_documents').select('*').eq('claim_id', claimId);
  if (pErr) throw new Error(`settlementDocumentService: prior lookup failed — ${pErr.message}`);
  const sameKindFiled = (priors || []).filter(d => d.package_kind === kind && d.status === 'filed');
  const version = (priors || [])
    .filter(d => d.package_kind === kind)
    .reduce((m, p) => Math.max(m, p.version || 1), 0) + 1;

  // ATOMIC replacement order: store the new version FIRST; only after it
  // is durably filed do the priors supersede. An insert failure leaves
  // the current version untouched; a supersede failure leaves both
  // filed (fail-safe — the higher version wins) and is logged loudly.
  const row = {
    ...
    ai_summary: null,
    relevant_to: [],
    pdf_buffer_b64: rendered.buffer.toString('base64'),
    triage_status: 'none',
    version,
    created_at: now,
    updated_at: now,
  ...
  for (const p of sameKindFiled) {
    const { error: sErr } = await supabase.from('claim_documents')
      .update({ status: 'superseded', superseded_by: document.id, updated_at: now })
      .eq('id', p.id).eq('status', 'filed');
    if (sErr) {
      logger.error({
        msg: 'settlementDocumentService: supersede of prior version failed — both versions remain filed',
        claimId, kind, prior: p.id, current: document.id, err: sErr.message,
      });
    }
  }
  return document;
...
/**
 * When the screen requires an MSA, the request cannot just SAY one is
 * included — the filed MSA document itself must be verified: it exists,
 * belongs to this claim, is current (not superseded/rejected), and is
 * identifiable as an MSA. The verified document is linked to the
 * package (event + rendition).
 */
async function _verifyMsaDocument(claimId, msaDocumentId) {
  if (!msaDocumentId) {
    throw new Error(
      'CNR_PACKAGE_BLOCKED_MSA_REQUIRED — the screen requires an MSA; ' +
      'link the filed MSA document (msa_document_id) to generate');
  }
  const { data: doc, error } = await supabase
    .from('claim_documents').select('*').eq('id', msaDocumentId).single();
  if (error || !doc) {
    throw new Error('CNR_PACKAGE_BLOCKED_MSA_DOCUMENT — linked MSA document not found');
  }
  if (doc.claim_id !== claimId) {
    throw new Error('CNR_PACKAGE_BLOCKED_MSA_DOCUMENT — linked MSA document is not filed on this claim');
  }
  if (doc.status !== 'filed') {
    throw new Error(`CNR_PACKAGE_BLOCKED_MSA_DOCUMENT — linked MSA document is not current (status: ${doc.status})`);
  }
  const looksLikeMsa = doc.package_kind === 'msa' ||
    /(^|[^a-z])msa([^a-z]|$)|medicare set[- ]?aside/i.test(String(doc.title || ''));
  if (!looksLikeMsa) {
    throw new Error('CNR_PACKAGE_BLOCKED_MSA_DOCUMENT — linked document is not identifiable as an MSA');
  }
  return doc;
}
...
async function generateCnRPackage(claimId, opts = {}) {
  ...
  // M19 deterministic gate, re-enforced at the document boundary. The
  // request's say-so (the old msa_included flag) is NOT trusted: when
  // the screen requires an MSA, the actual filed MSA document is
  // verified and linked to the package.
  ...
  let msaDocument = null;
  if (msa.msa_required) {
    msaDocument = await _verifyMsaDocument(claimId, opts.msa_document_id);
  }
  ...
  if (bd.available && !bd.sumMatches) {
    throw new Error(
      'CNR_PACKAGE_BLOCKED_BREAKDOWN_MISMATCH — the itemized breakdown does not equal ' +
      `the C&R total (${fmt$(offer.cnr_value)}); correct the offer before generating`);
  }
  ...
  const sections = [
    ...
    ['Settlement amount', [
      ...
      ...(bd.available ? [
        ...
        `  Breakdown source: ${bd.source} (sum-validated against the total)`,
      ] : ['  Breakdown: single-line (DN85 500 fallback) — itemize at OACR']),
    ]],
    ['Medicare interests (deterministic screen)', [
      `MSA screen ${msa.id}: msa_required=${!!msa.msa_required}${msa.msa_required ? '' : ' — no Medicare interest identified'}`,
      ...(msaDocument ? [
        `MSA document (verified on this claim): "${msaDocument.title}" — ${msaDocument.id} (v${msaDocument.version || 1})`,
      ] : []),
    ]],
  ...
  const rendered = await _renderPackage('Compromise and Release — DWC-CA 10214(c) [DRAFT RENDITION]', claim, sections);
  const document = await _fileVersioned(claimId, 'Compromise & Release package', rendered, 'cnr_10214c');

  const { error: evErr } = await supabase.from('claim_events').insert({
    claim_id: claimId, type: 'settlement_package_generated',
    timestamp: new Date().toISOString(),
    data: {
      kind: 'cnr_10214c', document_id: document.id, version: document.version,
      offer_id: offer.id, msa_document_id: msaDocument ? msaDocument.id : null,
    },
  });
  if (evErr) logger.error({ msg: 'settlementDocumentService: event insert failed', claimId, err: evErr.message });
  ...
  return {
    ...
    msa_document_id: msaDocument ? msaDocument.id : null,
    breakdown: bd,
    represented,
  };
}
...
async function generateStipPackage(claimId) {
  ...
  const rendered = await _renderPackage('Stipulations with Request for Award — DWC-CA 10214(a) [DRAFT RENDITION]', claim, sections);
  const document = await _fileVersioned(claimId, 'Stipulations with Request for Award package', rendered, 'stip_10214a');
