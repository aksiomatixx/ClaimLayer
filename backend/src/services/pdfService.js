...
/**
 * Render a stored claim document as a simple PDF (demo documents have no
 * binary — the original lives in the retained system of record; this gives
 * the drawer's "open original" link a real artifact to open).
 */
async function generateClaimDocumentPDF(doc, claim) {
  ...
  line(doc.title || 'Claim document', bold, 18, dark); y -= 6;
  line(`Claim ${claim?.claimNumber || doc.claim_id} · ${doc.category || 'document'} · received ${String(doc.received_at || '').split('T')[0]} · ${doc.pages || 1} page(s) · source: ${doc.source || 'unknown'}`, font, 10, gray);
  y -= 18;
  line('AI SUMMARY', bold, 11, dark); y -= 2;
  line(doc.ai_summary || 'No summary available.', font, 11.5, dark);
  y -= 24;
  line('This is a synthetic demo document generated by ClaimLayer. In production, this link opens the original file from the retained system of record.', font, 9, gray);
  return Buffer.from(await pdf.save());
}

module.exports = { generateDWC1, generateAuthorizationLetter, generateReasoningPDF, generateClaimDocumentPDF };
